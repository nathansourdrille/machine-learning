...
def plot_decision_regions_2d(X, Y, classifier, resolution=0.02, title=' '):
    ...
    cs = plt.contour(xx1, xx2, Z, levels=np.array([0]), colors = "g", alpha=0.9, linewidths=4)
...
def binarize_ground_truth(ground_truth):
    gt_mod = np.empty_like(ground_truth)
    tresh = 0.0025
    for i in range(gt_mod.shape[0]):
        for j in range(gt_mod.shape[1]):
            if ground_truth[i, j] > tresh:
                gt_mod[i,j] = 1.0
            else:
                gt_mod[i,j] = 0.0
    return gt_mod